...
def is_safe(gt_image, row, col, count ,row_size, col_size, channel):
...
def itr_DFS(gt_image, row, col, count, row_size, col_size, channel, x_points, y_points):
...
def target_sign(GTfile_path, img_data_obj, channel, good_bands):
    print("calculating target sign from ground truth")
    source_count = 0
    norm_value = 0
    t_mean = np.zeros((channel,), dtype=np.float64)
#    gt_image = cv2.imread(GTfile_path)
...
def sort_coordinates(x_points, y_points):
    if len(x_points)>1:
        mid = len(x_points)//2
        L_x = x_points[:mid]
        L_y = y_points[:mid]
...
def enclosing_rect_mean(x_points, y_points, img_data_obj, good_bands):
    min_x = min(x_points)
    max_x = max(x_points)
    min_y = min(y_points)
    max_y = max(y_points)
#    med_x = int(statistics.median(x_points))
#    med_y = int(statistics.median(y_points))
#    print("Sorting x-cooridinates..")

#    x_points,y_points = sort_coordinates(x_points, y_points)
...
#           aplha(x) = a_hat = (transpose(x-u) . inv_cov . (t-u) / (transpose(t-u) . inv_cov . (t-u))
            print("b_img_data", b_img_data[:,columns:col_range,:].shape, "target_mean : ", target_mean[start_pos:start_pos+bands_in_set].shape) 
            alpha = np.concatenate((alpha, matched_filter(b_img_data[:,columns:col_range,:], target_mean[start_pos:start_pos+bands_in_set], b_mean_cov_obj)), axis=1)
...
#%% writing image to envi format if needed use this
def envi_writer():    
    img = np.load("/media/bisque/My Passport/Alana/ang20150419t221127_rdn_v1f_clip/ang20150419t221127_rdn_v1f_clip_ch4.npy")
    envi.save_image('img1127.hdr', img, dtype=np.float32)
